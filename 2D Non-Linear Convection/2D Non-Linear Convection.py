...
#-------------2D Non-Linear Convection Equation---------------#
...
v0=u0_gaussian_2d(X,Y,0.3,0.5,0.1,0.1)
...
while t<tend:
    ...
    unew[1:-1,1:-1]=un[1:-1,1:-1] - 0.5*(un[1:-1,1:-1]+abs(un[1:-1,1:-1]))*(dt/dx)*(un[1:-1,1:-1] - un[1:-1,0:-2]) - 0.5*(vn[1:-1,1:-1]+abs(vn[1:-1,1:-1]))*(dt/dy)*(un[1:-1,1:-1] - un[0:-2,1:-1])\
                                  - 0.5*(un[1:-1,1:-1]-abs(un[1:-1,1:-1]))*(dt/dx)*(un[1:-1,2:] - un[1:-1,1:-1]) - 0.5*(vn[1:-1,1:-1]-abs(vn[1:-1,1:-1]))*(dt/dy)*(un[2:,1:-1] - un[1:-1,1:-1])      # backwar scheme and forward scheme or Generalized Upwind Scheme
    ...
    vnew[1:-1,1:-1]=vn[1:-1,1:-1] - 0.5*(un[1:-1,1:-1]+abs(un[1:-1,1:-1]))*(dt/dx)*(vn[1:-1,1:-1] - vn[1:-1,0:-2]) - 0.5*(vn[1:-1,1:-1]+abs(vn[1:-1,1:-1]))*(dt/dy)*(vn[1:-1,1:-1] - vn[0:-2,1:-1])\
                                  - 0.5*(un[1:-1,1:-1]-abs(un[1:-1,1:-1]))*(dt/dx)*(vn[1:-1,2:] - vn[1:-1,1:-1]) - 0.5*(vn[1:-1,1:-1]-abs(vn[1:-1,1:-1]))*(dt/dy)*(vn[2:,1:-1] - vn[1:-1,1:-1])      # backwar scheme and forward scheme or Generalized Upwind Scheme
...
ani = animation.FuncAnimation(fig, update, frames=len(u_sol), interval=1)
#fig.tight_layout()
ani.save(f"2D Non-Linear Convection/plots/2D_NLC_ug_vg.gif", writer="pillow")
plt.show()
